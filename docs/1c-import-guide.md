...
- Аутентификация: `secret` обязателен. Для `POST` передаётся в теле, для `GET` — в query (`?secret=...`). Значение должно совпадать с `ONEC_SECRET` в `.env` backend (см. `.env.dev/.env.production/.env.test`).
...
- `POST /api/1c/product-prices/batch`
- `POST /api/1c/special-prices/batch`
- `POST /api/1c/stock/batch`
- `GET /api/1c/orders/queued?secret=...`
- `POST /api/1c/orders/:guid/ack`
...
```
Если `guid` передан — upsert по нему. Если `guid` нет, используется уникальный ключ `(productId, priceTypeId, startDate)`. Поле `priceTypeGuid` можно опускать для «общего» прайса по товару.
...
### 3.8 Выгрузка заказов в 1С (очередь)
`GET /api/1c/orders/queued?secret=...`

Параметры:
- `limit` — по умолчанию 50, максимум 200
- `includeSent=true` — дополнительно включать `SENT_TO_1C`
...
```json
{
  ...
  "count": 1,
  "orders": [
    { "guid": "ORDER_GUID", "status": "QUEUED", "items": [] }
  ]
}
```

### 3.9 Подтверждение приёма заказа 1С
`POST /api/1c/orders/:guid/ack`
```json
{
  "secret": "ONEC_SECRET_VALUE",
...
```
Обновляет статус, номер/дату 1С, комментарий и сумму заказа.

### 3.11 Журнал синхронизации (диагностика)
- `GET /api/1c/sync/runs?secret=...`
- `GET /api/1c/sync/runs/:runId?secret=...&includeItems=true`

Можно фильтровать по `entity`, `direction`, `status` и ограничивать `limit`.
